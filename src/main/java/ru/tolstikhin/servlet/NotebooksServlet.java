...
@WebServlet(name = "NotebooksServlet", value = "/laptops")
public class NotebooksServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (address != null) {
            ...
            priceLists = serviceDAO.getPriceList(addressId, 3);
            request.setAttribute("priceLists", priceLists);
        }
        request.getRequestDispatcher("/views/notebooks.jsp").forward(request, response);
    }
